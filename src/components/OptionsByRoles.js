...
const OptionsByRoles = () => {
    const [optionsByRoles, setOptionsByRoles] = useState([{
        ...
        },
        {
            id: 2,
            role: 'User',
            options: ['Información Básica Trabajador', 'Recursos Humanos']
        },
        {
            id: 3,
            role: 'Supervisor',
            options: ['Nóminas', 'Tablas']
        },
    ]);
    ...
    const handleSave = (data) => {
        if (currentData) {
            setOptionsByRoles(optionsByRoles.map(optionByRole => optionByRole.id === currentData.id ? {
                ...optionByRole,
                ...data
            } : optionByRole));
        } else {
            setOptionsByRoles([...optionsByRoles, {
                id: optionsByRoles.length + 1,
                ...data
            }]);
        }
        setIsFormOpen(false);
    ...
    return ( <
        ...
        tbody > {
            filteredOptionsByRoles.map(optionByRole => ( <
                ...
                td className = "py-2 px-4 border-b border-gray-200" > {
                    optionByRole.options.join(', ')
                } < /td> <
                td className = "py-2 px-4 border-b border-gray-200" >
                <
                button onClick = {
                    () => handleEdit(optionByRole)
                }
                className = "bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-1 px-2 rounded mr-2" >
                Editar <
                /button> <
                button onClick = {
                    () => handleDelete(optionByRole.id)
                }
